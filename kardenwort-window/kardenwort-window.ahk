...
InjectHoverHighlightMvp(guiObj, bookmarksN) {
    try {
        ...
        css := ""
        css .= "body.dark #source-container span.word.hl-mvp-hover,"
        css .= "body.dark #translation-container span.word.hl-mvp-hover,"
        css .= "body.white #source-container span.word.hl-mvp-hover,"
        css .= "body.white #translation-container span.word.hl-mvp-hover {"
        css .= "  background-color: #39c5ff !important;"
        css .= "  color: #000000 !important;"
        css .= "}"
        css .= "body.light #source-container span.word.hl-mvp-hover,"
        css .= "body.light #translation-container span.word.hl-mvp-hover {"
        css .= "  background-color: #0969da !important;"
        css .= "  color: #ffffff !important;"
        css .= "}"
        css .= "body.dark #source-container span.word.hl-mvp-pin,"
        css .= "body.dark #translation-container span.word.hl-mvp-pin {"
        css .= "  background-color: #f0883e !important;"
        css .= "  color: #000000 !important;"
        css .= "}"
        css .= "body.light #source-container span.word.hl-mvp-pin,"
        css .= "body.light #translation-container span.word.hl-mvp-pin,"
        css .= "body.white #source-container span.word.hl-mvp-pin,"
        css .= "body.white #translation-container span.word.hl-mvp-pin {"
        css .= "  background-color: #bf8700 !important;"
        css .= "  color: #ffffff !important;"
        css .= "}"
        ...
        js .= "    if (el.className.indexOf(name) === -1) {"
        js .= "      el.className += ' ' + name;"
        js .= "      el.className = el.className.replace(/\s+/g, ' ').replace(/^\s+|\s+$/g, '');"
        js .= "    }"
        js .= "  }"
        js .= "  function removeClass(el, name) {"
        js .= "    var reg = new RegExp('(\\s|^)' + name + '(\\s|$)', 'g');"
        js .= "    el.className = el.className.replace(reg, ' ').replace(/\s+/g, ' ').replace(/^\s+|\s+$/g, '');"
        js .= "  }"
        ...
        js .= "      if (firstChild && firstChild.nodeType === 1 && firstChild.tagName === 'SPAN' && (firstChild.className || '').indexOf('word') !== -1) {"
        js .= "        if (firstChild.className.indexOf('hl-mvp') !== -1) continue;"
        js .= "        var childSpans = div.getElementsByTagName('span');"
        js .= "        for (var j = 0; j < childSpans.length; j++) {"
        js .= "          var span = childSpans[j];"
        js .= "          if ((span.className || '').indexOf('word') !== -1) {"
        ...
        js .= "      if (srcSpans.length > 0) {"
        js .= "        isIndexBuilt = true;"
        js .= "      }"
        js .= "      for (var i = 0; i < srcSpans.length; i++) {"
        js .= "        var span = srcSpans[i];"
        js .= "        if ((span.className || '').indexOf('word') !== -1) {"
        js .= "          var lc = span.getAttribute('data-lower-clean');"
        js .= "          if (!lc) {"
        js .= "            var txt = span.textContent || span.innerText || '';"
        js .= "            var pts = tokenizeText(txt); lc = pts.length > 1 ? pts[1].toLowerCase() : txt.toLowerCase();"
        js .= "            span.setAttribute('data-lower-clean', lc);"
        js .= "          }"
        js .= "          if (lc) {"
        js .= "            if (!lcIndex[lc]) lcIndex[lc] = { source: [], translation: [] };"
        js .= "            lcIndex[lc].source.push(span);"
        js .= "          }"
        js .= "        }"
        js .= "      }"
        js .= "    }"
        ...
        js .= "        if ((span.className || '').indexOf('hl-mvp') !== -1) {"
        js .= "          var lc = span.getAttribute('data-lower-clean');"
        js .= "          if (!lc) {"
        js .= "            var txt = span.textContent || span.innerText || '';"
        js .= "            var pts = tokenizeText(txt); lc = pts.length > 1 ? pts[1].toLowerCase() : txt.toLowerCase();"
        ...
        js .= "          if (lc) {"
        js .= "            if (!lcIndex[lc]) lcIndex[lc] = { source: [], translation: [] };"
        js .= "            lcIndex[lc].translation.push(span);"
        js .= "          }"
        js .= "        }"
        js .= "      }"
        js .= "    }"
        ...
        js .= "      ensureIndex();"
        js .= "      var lc = this.getAttribute('data-lower-clean');"
        js .= "      if (lc && lcIndex[lc]) {"
        js .= "        var isSource = (this.className.indexOf('hl-mvp') === -1);"
        js .= "        var targets = isSource ? lcIndex[lc].translation : lcIndex[lc].source;"
        js .= "        for (var i = 0; i < targets.length; i++) {"
        js .= "          addClass(targets[i], 'hl-mvp-hover');"
        js .= "        }"
        js .= "      }"
        js .= "    };"
        ...
        js .= "        var isSource = (this.className.indexOf('hl-mvp') === -1);"
        js .= "        var targets = isSource ? lcIndex[lc].translation : lcIndex[lc].source;"
        js .= "        var isPinned = false;"
        js .= "        for (var b = 0; b < bookmarks.length; b++) {"
        js .= "          if (bookmarks[b].lc === lc) { isPinned = true; break; }"
        js .= "        }"
        js .= "        for (var i = 0; i < targets.length; i++) {"
        js .= "          if (!isPinned) removeClass(targets[i], 'hl-mvp-hover');"
        js .= "        }"
        js .= "      }"
        js .= "    };"
        ...
        js .= "        if (bookmarks[i].lc === lc) { idx = i; break; }"
        js .= "      }"
        js .= "      if (idx !== -1) {"
        js .= "        var entry = bookmarks[idx];"
        js .= "        for (var j = 0; j < entry.spans.length; j++) {"
        js .= "          removeClass(entry.spans[j], 'hl-mvp-pin');"
        js .= "        }"
        js .= "        bookmarks.splice(idx, 1);"
        js .= "      } else {"
        js .= "        var spans = [];"
        js .= "        if (lcIndex[lc]) spans = lcIndex[lc].source.concat(lcIndex[lc].translation);"
        js .= "        for (var j = 0; j < spans.length; j++) {"
        js .= "          addClass(spans[j], 'hl-mvp-pin');"
        js .= "        }"
        js .= "        bookmarks.push({ lc: lc, spans: spans });"
        js .= "        if (bookmarks.length > N) {"
        js .= "          var oldest = bookmarks.shift();"
        js .= "          for (var j = 0; j < oldest.spans.length; j++) {"
        js .= "            removeClass(oldest.spans[j], 'hl-mvp-pin');"
        js .= "          }"
        js .= "        }"
        js .= "      }"
        js .= "    };"
        ...
        js .= "        for (var j = 0; j < entry.spans.length; j++) {"
        js .= "          removeClass(entry.spans[j], 'hl-mvp-pin');"
        js .= "          removeClass(entry.spans[j], 'hl-mvp-hover');"
        js .= "        }"
        js .= "      }"
        js .= "      bookmarks = [];"
        js .= "    }"
